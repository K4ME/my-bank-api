...
router.post("/create/:name/:balance", async (req, res) => {
  try {
    ...
    logger.info(
      `POST /account/create/:name/:balance - ${JSON.stringify(account)}`
    );
  } catch (err) {
...
router.put("/deposit/:id/:deposit", async (req, res) => {
  try {
    ...
    logger.info(
      `PUT /account/deposit/:id/:deposit- " ${JSON.stringify(grade)}`
    );
  } catch (err) {
    res.status(400).send({ error: err.message });
  }
...
router.put("/withdraw/:id/:withdraw", async (req, res) => {
  try {
    ...
    logger.info(
      `PUT /account/withdraw/:id/:withdraw - " ${JSON.stringify(grade)}`
    );
  } catch (err) {
    res.status(400).send({ error: err.message });
  }
